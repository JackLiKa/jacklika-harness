...
- `@deepseek-ai/dsh-memory-queue` —— 通过 `tools/execute` waterfall 把 `wiki_write`（可配置）调用串行化，提供单写者顺序；可选的 `crossProcessLock` 在仓库根持有心跳刷新的 `mkdir` 锁目录，使不同 dsh 进程也无法交错写入，且慢速写入不会被误判过期回收。`wiki_write` 通过临时文件 + `rename` 原子发布。
