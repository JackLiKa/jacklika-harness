...
- `@deepseek-ai/dsh-memory-queue` —— 通过 `tools/execute` waterfall 把 `wiki_write`（可配置）调用串行化，提供单写者顺序；可选的 `crossProcessLock` 在仓库根持有 `mkdir` 锁目录，其活性由心跳计数器文件证明——与机器时钟无关的变化检测，使不同 dsh 进程无法交错写入，慢速写入不会被误回收，跨机器时钟偏差也无法伪造过期。`wiki_write` 通过临时文件 + `rename` 原子发布。
