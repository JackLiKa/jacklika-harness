...
- `@deepseek-ai/dsh-memory` — a profile bundle whose `cordis.patch.yml` mounts the whole chain in waterfall order (scope → queue → git → filesystem → graph, vector disabled without an endpoint). The shipped `memory` profile template composes `dsh-base` + this bundle + `dsh-headless`, so `dsh --profile memory "<task>"` runs a one-shot CLI session with memory enabled; any custom profile can list the bundle after `dsh-base` instead.
