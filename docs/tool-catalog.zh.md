...
| `@deepseek-ai/dsh-tool-memory-filesystem` | `wiki_read`、`wiki_search`、`wiki_write` | `ctx.tools`、`ctx.systemPrompt（经 ToolRuntime 传递）` | `tool/call`、`tool/result` | - | 基于文件系统的 wiki/记忆工具：wiki_read 解析 YAML frontmatter 并跟随 Obsidian 风格 `[[link]]` 链接，wiki_search 在仓库中做关键词搜索，wiki_write 创建或追加笔记。所有路径都被约束在配置的 vaultRoot 内。 |
...
读取 wiki 仓库中的一条 Markdown 笔记，可选择按配置深度跟随 Obsidian 风格 `[[link]]` 链接。返回笔记 id、frontmatter、正文和已解析的链接笔记。
...
来源：[`packages/fs/tool-memory-filesystem/src/index.ts`](../packages/fs/tool-memory-filesystem/src/index.ts)

### `wiki_search`

按笔记标题或正文关键词搜索 wiki 仓库。返回匹配的笔记 id、标题和反向链接数量。在询问用户该读哪条笔记前先使用本工具。
...
在 wiki 仓库中创建新笔记或追加到已有笔记。路径相对于仓库根目录。追加模式下，新内容会插入正文末尾，并带有一个时间戳标题。
...
基于文件系统的 wiki/记忆工具：wiki_read 解析 YAML frontmatter 并跟随 Obsidian 风格 `[[link]]` 链接，wiki_search 在仓库中做关键词搜索，wiki_write 创建或追加笔记。所有路径都被约束在配置的 vaultRoot 内。
